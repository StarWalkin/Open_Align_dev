...
def exchange_to_ori_label(exchanged_pred_label):
    ...
    if not(exchanged_pred_label in [0, 1, 2]):
        print(f"error!exchanged_pred_label:{exchanged_pred_label}")
        print(counter)
...
if __name__ == '__main__':
    ...
    #exception: pred = -1
    ##出bug了！这样会跳过一个，如果连续两个-1会寄掉！！有意思
    # for idx, pred_data in enumerate(pred):
    idx = 0
    while (idx < (len(pred))):
        if(idx >= len(pred)):
            break
        # if pred[idx]["output"] not in [0,1,2] or pred_exchange[idx]["output"] not in [0,1,2]:
        #     print("-------------------------")
        #     print(pred[idx])
        #     print(pred_exchange[idx])
        #     source.pop(idx)
        #     pred.pop(idx)
        #     pred_exchange.pop(idx)
        #     print(f"reomved line {idx}" )
        #     print(pred[idx])
        #     print(pred_exchange[idx])
        #     idx -= 1
        # idx+=1
        if pred[idx]["output"] not in [0,1,2] or pred_exchange[idx]["output"] not in [0,1,2]:
            print("-------------------------")
            if pred[idx]["output"] not in [0,1,2]:
                pred[idx]["output"] = random.randint(0, 2)
            if pred_exchange[idx]["output"] not in [0,1,2]:
                pred_exchange[idx]["output"] = random.randint(0, 2)
            idx -= 1
        idx+=1
    print(len(pred))
    print(len(pred_exchange))
