...
mapping_chat = {
    ...
    "gemma-2b-it": {"path": f"{ckpt_dir}/google/gemma-2b-it", "wrapper_type": "gemma","AB_token_ids":[235280, 235305]},
}
...
if __name__ == '__main__':
    ...
    data = read_all("/cpfs01/user/liupengfei/jlli/pjs/alignment-decomposition/shared/data/chatbot_arena_shuffled_no-tie_group_balanced.jsonl")

    if way == "get_prompt_probs":
        all_wrapped_q_rs = []
        prompt_response_tokens = []
        start_pos = len(read_jsonl(f"/cpfs01/user/liupengfei/jlli/pjs/alignment-decomposition/collected_data/model_preference/{model_name}/query_responses_logprobs.jsonl"))
        ...
        write_jsonl(transformed_logprobs,
                    f"/cpfs01/user/liupengfei/jlli/pjs/alignment-decomposition/collected_data/model_preference/{model_name}/query_responses_logprobs.jsonl",mode="a")
    elif way == "binary_choice":
        all_wrapped_q_rpairs = []
        AB_flag = "AB" if not args.change_AB else "BA"
        start_pos = len(read_all(f"/cpfs01/user/liupengfei/yguo/Open_Align_dev/preference_dissection/{model_name}/direct_ask_{args.ask_AB_prompt}_preferences_{AB_flag}.jsonl"))
        for item in data[start_pos:args.end_pos]:
            query = item["prompt"]
            r1 = item["response_a"]
            r2 = item["response_b"]
