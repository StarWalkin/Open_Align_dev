...
class AnthropicEngine():
    def __init__(self, api_key, model="claude-2"):
        self.api_key = api_key
        self.engine = Anthropic(api_key=api_key)
        self.model = model

    def generate_single(self, raw_query, max_new_tokens, temperature, top_p):
        try:
            completion = self.engine.completions.create(
                model=self.model,
                max_tokens_to_sample=max_new_tokens,
                prompt=f"{HUMAN_PROMPT} {raw_query}{AI_PROMPT}",
                temperature=temperature,
                top_p=top_p,
            )
        except:
            ...
            return self.generate_single(raw_query, max_new_tokens, temperature, top_p)
