...
class BayesianLogisticRegression:
    ...
    def predict(self, X):
        ...
        return np.round(np.exp(logits) / (1 + np.exp(logits)))
...
def fit_logisticregression(X, y, model_name, show=True, penalty=None, add_bias=False, C=1.0, ):
    if penalty == 'l1':
        model = sklearn.linear_model.LogisticRegression(fit_intercept=add_bias, penalty=penalty, solver='liblinear',
                                                        C=C)
    else:
        model = sklearn.linear_model.LogisticRegression(fit_intercept=add_bias, penalty=penalty, C=C)
    model.fit(X, y)
...
if __name__ == '__main__':
    ...
    label_data = read_all(
        "./collected_data/model_preference/labels/chatbot_arena_shuffled_no-tie_group_balanced_labels.jsonl")
    ...
    for k in key_list:
        all_xx = read_all(f"./collected_data/model_preference/fitted_paras/model_{k}_fitted_paras.jsonl")
        ...
        for model_name in ["human"] + os.listdir("./collected_data/model_preference"):
            # if model_name not in "gpt-4-1106-preview": continue
            listxx = havemapping.get(model_name, ["inst_naive", "inst_naive_by-exchange"])
            for further_divide in listxx:
                ...
                for idx, item in enumerate(resolved_data):
                    ...
                    try:
                        ...
                        inter = True
                        break
                    feature = get_feature(item, remove_length=False)
                ...
                model, parameters = fit_regression(features_train, labels_train, method=fitting_method,
                                                   ...
                                                   scale=0.1, penalty=None, C=0.1, suffix=k)
        ...
        write_jsonl(all_xx, f"./collected_data/model_preference/fitted_paras/model_{k}_fitted_paras.jsonl", mode="a")
