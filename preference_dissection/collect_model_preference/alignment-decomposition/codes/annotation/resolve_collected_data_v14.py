...
if __name__ == '__main__':
    show_raw = False
    source = read_all("./shared/data/chatbot_arena_shuffled_no-tie_group_balanced.jsonl")
    version_tag = "v1.4.0_notie_pair"
    output_tag = "v1.4.0_resolved"
    collected_data_dir = f"./collected_data/{version_tag}"
    ...
    for idx, (s, qinfo, q0f, q1f, q2f, acc) in enumerate(zip(source, query_infos, query_free_features_by_pair,
        ...
        if show_raw:
            print_colored_text("query_info", color="green")
            elegant_show(r_qinfo)
            print_colored_text("query_free", color="green")
            elegant_show(r_q0f)
            print_colored_text("query_aware_once", color="green")
            elegant_show(r_q1f)
            print_colored_text("query_aware_twice", color="green")
            elegant_show(r_q2f)
            print_colored_text("accuracy", color="green")
            elegant_show(r_acc)
            print_colored_text("====================================", color="purple")
    ...
    write_jsonl(new_output, f"./collected_data/{output_tag}/chatbot_arena_no-tie_group_balanced_resolved.jsonl")
    write_json(all_query_info_list, f"./collected_data/{output_tag}/query_aware_idxs.json")
